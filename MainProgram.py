...
class MainWindow(QMainWindow):
    ...
    def initMain(self):
        ...
        #使用YOLOv8框架，通过ultralytics提供的API加载了一个模型权重Config.model_path（models/best.pt）
        ...
        image_path = os.path.join(os.getcwd(), "UIProgram", "ui_imgs", "11.png")
        if os.path.exists(image_path):
            pixmap = QPixmap(image_path).scaled(
                self.ui.label_show.width(),
                self.ui.label_show.height(),
    ...
    def camera_show(self):
        ...
        if self.is_camera_open:
            ...
            if not self.cap.isOpened():
                ...
                QMessageBox.warning(self, '错误', '无法打开摄像头！请检查是否被占用或驱动异常')
                self.ui.CaplineEdit.setText('摄像头打开失败')
                self.is_camera_open = False
                return
            ...
            # 运行程序：保存并重新运行你的程序，点击摄像头按钮
            # 验证功能：
            # 如果摄像头能正常打开，显示"摄像头开启"
            # 如果无法打开摄像头，弹出提示框告知用户具体错误（如被占用或驱动异常）
